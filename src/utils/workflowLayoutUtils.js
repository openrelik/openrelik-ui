 ...
 */
export const computeWorkflowLayout = (workflowData, taskStatusMap, options = {}) => {
  ...
  const processTaskList = (tasks, parentId, x, y, depth = 0) => {
    ...
    tasks.forEach((task, index) => {
      if (task.type === "chord") {
        ...
        const branchesHeight = processTaskList(branches, parentId, x, currentSiblingY, depth);
        ...
        if (callbackTask) {
          ...
          processTaskList([callbackTask], null, callbackX, callbackY, depth + 1);
      ...
      if (index > 0) {
        ...
        const isPrevComplex = prevTask.type === 'chord' || 
                              (prevTask.tasks && prevTask.tasks.length > 1) ||
                              (prevTask.tasks && prevTask.tasks.length > 0 && prevTask.tasks[prevTask.tasks.length - 1].type === 'chord');

        const spacing = (isGroupBoundary || isPrevComplex) ? 120 : 20;
      ...
      const blockHeight = Math.max(selfHeight, childrenHeight);
