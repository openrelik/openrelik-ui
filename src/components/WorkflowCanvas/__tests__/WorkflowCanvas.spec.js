...
const WorkflowEdgeLayerStub = {
  name: "WorkflowEdgeLayer",
  template: "<div class='edge-layer-stub'></div>",
};
const WorkflowNodeStub = {
  name: "WorkflowNode",
  template:
    "<div class='workflow-node-stub' @click=\"$emit('toggle-overview', node.id)\"></div>",
...
const WorkflowTaskSelectorStub = {
  ...
  template:
    "<div class='task-selector-stub' @click=\"$emit('select', {id:'t1'})\"></div>",
  emits: ["select", "close"],
};
...
describe("WorkflowCanvas.vue", () => {
  ...
  it("handles node interactions", async () => {
    ...
    expect(workflowStore.updateNodePosition).toHaveBeenCalledWith(
      "node-1",
      50,
      50
    );
  ...
  describe("Coverage Improvements", () => {
    ...
    describe("Workflow and Folder Renaming Sync", () => {
      it("emits workflow-renamed when workflow name changes from Untitled workflow", async () => {
        ...
        expect(wrapper.emitted("workflow-renamed")[0]).toEqual([
          "New Workflow Name",
        ]);
      });
